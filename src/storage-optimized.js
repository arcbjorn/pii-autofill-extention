...
export class OptimizedStorageManager {
    ...
    determineOptimalStorage(key, data) {
    ...
    async getBatch(keys, options = {}) {
        ...
        const promises = [];
        for (const [storageType, batchKeys] of Object.entries(batches)) {
            if (batchKeys.length > 0) {
                promises.push(chrome.storage[storageType].get(batchKeys).then(data => {
    ...
    queueWrite(key, data, storageType, options = {}) {
        const writeOp = {
            key,
            data,
            storageType,
        ...
        try {
            ...
            for (const [storageType, batch] of Object.entries(batches)) {
                if (Object.keys(batch).length > 0) {
                    promises.push(chrome.storage[storageType].set(batch).then(() => {
    ...
    async setChunked(key, data, storageType) {
        ...
        // Store chunks
        const chunkPromises = chunks.map((chunk, index) => chrome.storage[storageType].set({
    ...
    // Get chunked data
    async getChunked(key, storageType) {
        try {
            // Get metadata
    ...
    async remove(key, storageType = null) {
        if (!storageType) {
